...
#'
#' Adds labels beside specified points on a Manhattan or Miami plot. Ideal for adding locus names to peaks. Currently only designed to work with \code{\link[miamiPlot]{miamiPlot}}.
#'
...
#' @param chrmaxpos Data frame containing x coordinates for chromosome start positions, generated by \code{\link[miamiPlot]{labelManhattan}}.
...
#' @export
#' @examples labelManhattan(c(4,5,11,19),c(9994215,16717922,45538760,51699256),c("GENE1","GENE2","GENE3","GENE4"),gwas1,chrmaxpos=chrmaxpos)
#' labelManhattan(geneLabels$chr,gebeLabel$pos,geneLabel$geneName,gwas1,chrmaxpos=chrmaxpos)

labelManhattan <- function(chr,pos,name,gwas,gwasChrLab="chr",gwasPosLab="pos",gwasPLab="p",chrmaxpos,textPos=4,angle=0,miamiBottom=FALSE) {
   ...
   if(length(pCol)==0){
      stop(paste0("Could not find column ",gwasPLab," in input"))
   }

   if(class(chrmaxpos)!="data.frame"){
      stop("chrmaxpos must be an output from the miamiPlot function")
   }
